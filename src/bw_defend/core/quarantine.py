...
def _save_manifest(entries: list[dict[str, str]]) -> None:
    ...
    _manifest_path().write_text(json.dumps(entries, indent=2, sort_keys=True))
...
def quarantine_file(path: str) -> QuarantineItem:
    ...
    item_id = f"q-{int(datetime.now(timezone.utc).timestamp())}-{src.name}"
    dst = q_dir / item_id
...
def restore_item(item_id: str) -> dict[str, str]:
    ...
    for entry in entries:
        if entry["id"] == item_id:
            ...
            _save_manifest(entries)
            return entry
    raise ValueError(f"quarantine item not found: {item_id}")
